...
$entry_fields = array('create_by', 'book_by', 'name', 'description', 'start_time', 'end_time', 'room_id',
...
function get_field_book_by(string $value, $disabled=false)
{
  global $is_mandatory_field;

  $params = array('label'    => get_vocab('bookedby'),
    'name'     => 'book_by',
    'field'    => 'entry.book_by',
    'value'    => $value,
    'required' => !empty($is_mandatory_field['entry.book_by']),
    'disabled' => $disabled);
...
foreach ($edit_entry_field_order as $key)
{
  switch ($key)
  {
    ...
    case 'book_by':
      $fieldset->addElement(get_field_book_by($book_by));
      break;
