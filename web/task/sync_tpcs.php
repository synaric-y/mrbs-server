        ...
        foreach ($fmtChangeList["create"] as $create) {
          if (!empty($create['data']['repeat_rule'])) {
            ...
            foreach ($reps as $rep) {
              ...
              $one = DBHelper::one(\MRBS\_tbl("entry"), "room_id = {$create['data']['room_id']} AND ((start_time <= {$start} AND end_time > {$start}) OR (start_time < {$end} AND end_time >= {$end}) OR (start_time >= {$start} AND end_time < {$end}) OR (start_time < {$start} AND end_time >= {$end}))");
              if (!empty($one)) {
                foreach ($thirdCalendarService as $serviceName => $config) {
                  if ($area[$config["switch"]] != 1)
                    continue;
