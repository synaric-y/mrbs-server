...
function syncAD()
{
  ...
  foreach ($result as $group) {
    $fmtGroup = _handleGroup($group, $totalGroup);
    if (!empty($fmtGroup)) {
  ...
  foreach ($userList as $p) {
    try {
      ...
      $fmtUser = _handleUser($p, $totalUser, $fmtGroupList, $option);
      if (!empty($fmtUser)) {
        $fmtUserList[] = $fmtUser;
      }
...
//      log_ad("handle g2g: {$localGroup['name']}, parent: {$localGroup['third_parent_id']}";
  ...
  foreach ($localUserList as $key => $localUser) {
    try {
      $parentNodeList = [];
      recursiveGetParentList($localGroupList, $localUser, $parentNodeList, 0);
      if (empty($parentNodeList)) {
        continue;
      }
...
function _handleUser(User $user, &$total, array $originGroupList, $option)
{
  ...
  $result = array();
  $result['third_id'] = _GUIDtoStr($user->getObjectGuid());
...
//  $result['title'] = $user->getAttribute('title')[0];
  ...
  foreach ($groupList as $group) {
    $groupId = _GUIDtoStr($group->getObjectGuid());
...
function _handleGroup(Group $group, &$totalGroup)
{
  ...
  $result['third_id'] = _GUIDtoStr($group->getObjectGuid());
  $parent = array();

  $groupList = $group->groups()->getResults()->all();
  if (!empty($groupList)) {
    foreach ($groupList as $p) {
      $parent[] = _GUIDtoStr($p->getObjectGuid());
    }
  }
